...
public class Protot_CompileAsync_Should
{
    ...
    {
       Assert.Throws<ArgumentNullException>(() => new Protot(source, "target", new List<ProtoTransformation>()));
    }
    ...
    {
        Assert.Throws<ArgumentNullException>(() => new Protot("source", "target", transformationConfigurations));
    }
    
    [Fact]
    public void ThrowIfTransformationConfigurationIsEmpty()
    {
        Assert.Throws<ArgumentNullException>(() => new Protot("source", "target", Enumerable.Empty<ProtoTransformation>()));
    }
    ...
    public async Task ThrowIfProtoFileIsInvalid(string sourceFile, string targetFile)
    {
        ...
        var protot = new Protot(sourceFile, targetFile, transformations);
